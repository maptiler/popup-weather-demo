...
import { OrthographicCamera as Ye, Scene as He, RawShaderMaterial as Ke, GLSL3 as Xe, Vector2 as je, WebGLRenderTarget as Ae, LinearFilter as ce, RGBAFormat as ge, Mesh as qe, PlaneGeometry as Je, DataTexture as Qe, WebGLRenderer as et } from "three";
import tt from "ol/structs/LRUCache.js";
import { EventEmitter as ot } from "events";
var b, w, R, W, J, G, Q, Ee, _, F, ve, be, ee, Ce, te, Ie, oe, Me, me, ze, re, De;
class rt extends Map {
  ...
  resize(e) {
    if (!(e && e > 0))
      throw new TypeError("`maxSize` must be a number greater than 0");
    const t = [...P(this, re, De).call(this)], o = t.length - e;
  ...
  *entriesAscending() {
    for (const [e, t] of P(this, re, De).call(this))
      yield [e, t.value];
  }
  get size() {
    if (!d(this, b))
...
class Pe {
  ...
  constructor(i, e, t = yt()) {
    ...
    n(this, "size");
    this.size = new je(i, e), this.renderTarget = new Ae(i, e, {
      minFilter: ce,
      magFilter: ce,
      format: ge
   ...
   */
  setSize(i, e) {
    i === this.size.x && e === this.size.y || (this.size.x = i, this.size.y = e, this.renderTarget = new Ae(i, e, {
      minFilter: ce,
      magFilter: ce,
      format: ge
  ...
  updateSlippyTiles() {
    ...
    for (let M = 0; M < X; M++) {
      ...
      for (let z = 0; z < V; z++) {
        ...
        let C = this.getTilePlacement(r, !0, x, k, N), D = this.getTilePlacement(a, !0, x, k, N);
        if (C && !D && (D = C), !C && D && (C = D), !C || !D) {
          let ne = e;
          for (; !C && (ne = this.getNextFrame(ne, -1), !!ne); )
            C = D = this.getTilePlacement(
              ne.data,
              !1,
              x,
              k,
...
class Gt extends At {
  ...
  async onAdd(e, t) {
    ...
    this.colorRamp = this.constructorOptions.colorramp ?? A.colorramp.scale(0, 40), this.initParticle(
      {
      ...
      },
      {
        ...
        decodeAsWaves: !1,
        maxAmount: this.constructorOptions.maxAmount ?? A.maxAmount,
        color: this.constructorOptions.color ?? A.color,
        fastColor: this.constructorOptions.fastColor ?? A.fastColor,
        fastSpeed: this.constructorOptions.fastSpeed ?? A.fastSpeed,
        density: this.constructorOptions.density ?? A.density,
        size: this.constructorOptions.size ?? A.size,
        speed: this.constructorOptions.speed ?? A.speed,
        refreshInterval: this.constructorOptions.refreshInterval ?? A.refreshInterval,
        fadeFactor: this.constructorOptions.fadeFactor ?? A.fadeFactor,
        angleDirectionShiftSkip: 30,
        uniformSpeed: !0,
        fastIsLarger: this.constructorOptions.fastIsLarger ?? A.fastIsLarger
      },
      [
        new ie({
          decode: {
          ...
          smooth: this.constructorOptions.smooth ?? A.smooth,
          opacity: this.constructorOptions.opacity ?? A.opacity
        })
      ]
